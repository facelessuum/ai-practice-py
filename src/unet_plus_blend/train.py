"""Supervised training: python -m unet_plus_blend.train --help."""
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("runs/unet_plus_blend"))
    ...
    if min(args.epochs, args.base_channels, args.threads, args.depth) < 1:
        parser.error("epochs, base-channels, threads and depth must be positive")
    ...
    dataset = ExposureDataset(args.data, args.scale, min_size=2**args.depth)
    ...
    model = UNetPlusBlend(args.base_channels, depth=args.depth).to(args.device)
