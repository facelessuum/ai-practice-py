...
from .model import UNetPlusBlend
from .utils import check_output, create_run, latest_checkpoint


def load_model(path: Path, device: str = "cpu"):
    checkpoint = torch.load(path, map_location="cpu", weights_only=True)
    if checkpoint.get("architecture") != "unet_plus_blend":
        raise ValueError("Expected a unet_plus_blend checkpoint")
    model = UNetPlusBlend(checkpoint["base_channels"], depth=checkpoint["depth"])
...
def blend_case(model, case: Path, destination: Path, scale: float = 0.1):
    ...
    images = load_inputs(case, scale, min_size=2**model.depth)
    images = images.unsqueeze(0).to(next(model.parameters()).device)
...
def main():
    ...
    parser.add_argument("--model-root", type=Path, default=Path("runs/unet_plus_blend"))
    parser.add_argument("--output", type=Path, default=Path("output/unet_plus_blend"))
